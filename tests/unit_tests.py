...
def mock_resource_with(filename, resource_type):
    ...
    if resource_type not in VALID_RESOURCES:
        raise Exception(
            "Mocked resource must be one of: %s" % ", ".join(VALID_RESOURCES)
        )
    subfolder = "text" if resource_type == "txt" else "html"
    resource_path = os.path.join(
        TEST_DIR, "data/%s/%s.%s" % (subfolder, filename, resource_type)
    )
...
class ExhaustiveFullTextCase(unittest.TestCase):
    ...
    def test_exhaustive(self):
        ...
        total_pubdates_failed, total_fulltext_failed = list(
            map(sum, zip(*test_results))
        )

        print(
            "%s fulltext extractions failed out of %s"
            % (total_fulltext_failed, len(urls))
        )
        print(
            "%s pubdate extractions failed out of %s"
...
class ArticleTestCase(unittest.TestCase):
    ...
    def test_parse_html(self):
        ...
        TOP_IMG = (
            "http://i2.cdn.turner.com/cnn/dam/assets/131129200805-"
    ...
    def test_meta_type_extraction(self):
        ...
        meta_type = self.article.extractor.get_meta_type(self.article.clean_doc)
    ...
    def test_meta_extraction(self):
        ...
        META_DATA = defaultdict(
            dict,
            {
                ...
                "title": "After storm, forecasters see smooth "
                "sailing for Thanksgiving - CNN.com",
                "og": {
                    "site_name": "CNN",
                    "description": "A strong storm struck much of the "
                    "eastern United States on Wednesday, complicating "
                    "holiday plans for many of the 43 million Americans "
                    "expected to travel.",
                    "title": "After storm, forecasters see smooth "
                    "sailing for Thanksgiving",
                    "url": "http://www.cnn.com/2013/11/27/travel/weather-thanksgiving/index.html",
                    "image": "http://i2.cdn.turner.com/cnn/dam"
                    "/assets/131129200805-01-weather-1128-story-top.jpg",
                    "type": "article",
                },
                "section": "travel",
                "author": "Dana A. Ford, James S.A. Corey, "
                "Chien-Ming Wang, and Tom Watkins, CNN",
                "robots": "index,follow",
                "vr": {
                ...
                "keywords": "winter storm,holiday travel,"
                "Thanksgiving storm,Thanksgiving winter storm",
                "article": {"publisher": "https://www.facebook.com/cnninternational"},
                "lastmod": "2013-11-28T02:03:23Z",
                "twitter": {
                    "site": {"identifier": "@CNNI", "id": 2097571},
                    "card": "summary",
                    "creator": {"identifier": "@cnntravel", "id": 174377718},
                },
                "viewport": "width=1024",
                "news_keywords": "winter storm,holiday travel,"
                "Thanksgiving storm,Thanksgiving winter storm",
            },
        )
...
class SourceTestCase(unittest.TestCase):
    ...
    def test_source_build(self):
        ...
        s.clean_memo_cache()
        s.build()
        _ = (DESC, CATEGORY_URLS, FEEDS, BRAND)  # silence ruff
        ...
        # are primarily extracted from the HTML of category URLs. We lose this
    ...
    def test_cache_categories(self):
        ...
        _ = mock_resource_with("yahoo_main_site", "html")
        s = Source(url)
        s.download()
        s.parse()
        s.set_categories()
...
class MThreadingTestCase(unittest.TestCase):
    ...
    def test_download_works(self):
        ...
        print(
            (
                "Slate has %d articles TC has %d articles ESPN has %d articles"

...
def get_unicode_html(html):
    ...
    if not converted.unicode_markup:
        raise ValueError(
            "Failed to detect encoding of article HTML, tried: {}".format(", ".join(converted.tried_encodings))
        )
...
def get_tags_regex(
    ...
    selector = ".//{}[{}]".format(tag or "*", " and ".join(sel_list))
...
def get_tags(
    ...
    sel_list = []
    for k, v in attribs.items():
        trans = f'translate(@{k}, "{string.ascii_uppercase}", "{string.ascii_lowercase}")'

        if ignore_dashes:
            trans = f"translate({trans}, '-_', '  ')"

        if attribs_match == "exact":
            selector = f'{trans}="{v.lower()}"'
        elif attribs_match == "substring":
            selector = f'contains({trans}, "{v.lower()}")'
        elif attribs_match == "word":
            selector = f'contains(concat(" ", normalize-space({trans}), " "), " {v.lower()} ")'
...
def get_metatags(node: lxml.html.Element, value: str | None = None) -> list[lxml.html.Element]:
    ...
    selector = "//meta[{}]".format(" or ".join(sel_list))
    elems = node.xpath(selector)
    return elems
